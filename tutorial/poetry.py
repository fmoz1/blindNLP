...
with open(r'./datafiles/poetry/whitman.txt', 'r') as infile, \
        open(r'./datafiles/poetry/whitman_prepared.txt', 'w') as outfile:
    data = infile.read()
    data = data.replace("##", "")
    outfile.write(data)
...
input_texts = []
target_texts = []
for line in open('./datafiles/poetry/whitman_prepared.txt'):
    line = line.rstrip()
    if not line:
        continue
    input_line = '<sos> ' + line
...
# convert the sentences (strings) into ints
...
print('Max seq length: ', max_sequence_length_from_data)
...
# pad sequences and so we can get N x T matrix
...
initial_h = Input(shape=(LATENT_DIM,))  # initial hidden
initial_c = Input(shape=(LATENT_DIM,))  # initial cell
...
# reverse word2idx dic to get backwards
